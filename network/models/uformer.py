...
class Uformer(nn.Module):
    def __init__(self, in_channels=3, out_channels=3):
        ...
        self.pool1 = nn.Conv2d(32, 32, kernel_size=4, stride=2, padding=1)

        self.ConvBlock2 = ConvBlock(32, 64)
        self.pool2 = nn.Conv2d(64, 64, kernel_size=4, stride=2, padding=1)

        self.ConvBlock3 = ConvBlock(64, 128)
        self.pool3 = nn.Conv2d(128, 128, kernel_size=4, stride=2, padding=1)

        self.ConvBlock4 = ConvBlock(128, 256)
        self.pool4 = nn.Conv2d(256, 256, kernel_size=4, stride=2, padding=1)

        self.ConvBlock5 = ConvBlock(256, 512)

        # ===== Decoder (checkpoint: kernel=2) =====
    ...
    def _match_size(self, up, skip):
        if up.size(2) != skip.size(2) or up.size(3) != skip.size(3):
            th = min(up.size(2), skip.size(2))
            tw = min(up.size(3), skip.size(3))
            up = up[:, :, :th, :tw]
            skip = skip[:, :, :th, :tw]
        return up, skip
    ...
    def forward(self, x):
        ...
        up6 = self.upv6(conv5)
        up6, conv4 = self._match_size(up6, conv4)
        conv6 = self.ConvBlock6(torch.cat([up6, conv4], dim=1))

        up7 = self.upv7(conv6)
        up7, conv3 = self._match_size(up7, conv3)
        conv7 = self.ConvBlock7(torch.cat([up7, conv3], dim=1))

        up8 = self.upv8(conv7)
        up8, conv2 = self._match_size(up8, conv2)
        conv8 = self.ConvBlock8(torch.cat([up8, conv2], dim=1))

        up9 = self.upv9(conv8)
        up9, conv1 = self._match_size(up9, conv1)
        conv9 = self.ConvBlock9(torch.cat([up9, conv1], dim=1))
